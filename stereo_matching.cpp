...
Mat stereo_match(Mat left,Mat right,int window_size,int max_disparity,string cost) {
	int h = left.rows;
	int w = left.cols;
	...
	if (cost == "rank") {
		left = rank_transform(left, 7);
		right = rank_transform(right, 7);
	}
	else if (cost == "census") {
...
int main(int argc, char** argv)
{
	cv::CommandLineParser parser(argc, argv,
		"{left||}{right||}{max-disparity|0|}{output||}{parallel||}{cost||}");
	...
	if (parallel == "no"){
		dis = stereo_match(left, right, 6, max_disparity, cost);
	}
